...
@section('page_title')
    {{ __('agentadmin::app.settings.agent_sources.edit-title') }}
@stop
                    ...
                    <accordian :title="'{{ __('agentadmin::app.settings.agent_sources.general') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" id="name" name="name" data-vv-as="&quot;{{ __('agentadmin::app.settings.agent_sources.name') }}&quot;" value="{{ $user->name }}"/>
                                <span class="control-error" v-if="errors.has('name')">@{{ errors.first('name') }}</span>
                            </div>

                            <div class="control-group" :class="[errors.has('email') ? 'has-error' : '']">
                                <label for="email" class="required">{{ __('agentadmin::app.settings.agent_sources.email') }}</label>
                                <input type="text" v-validate="'required|email'" class="control" id="email" name="email" data-vv-as="&quot;{{ __('agentadmin::app.settings.agent_sources.email') }}&quot;" value="{{ $user->email }}"/>
                    ...
                    <accordian :title="'{{ __('agentadmin::app.settings.agent_sources.password') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('password') ? 'has-error' : '']">
                                ...
                                <input type="password" v-validate="'min:6|max:18'" class="control" id="password" name="password" data-vv-as="&quot;{{ __('agentadmin::app.settings.agent_sources.password') }}&quot;"/>
